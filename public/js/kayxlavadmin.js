...
$('.ajax-link').on('click', function(e) {
    ...
    $.fn.displayClickedItem(pageID);

});

$.fn.displayClickedItem = function(index) {


    if (index.toString() === "1") {
        $('#current-item').val("PageSections");
        event.preventDefault();
        var parentPage = 'Configuration';
        var currentPage = 'PageSections';
        var currentItem = 'Page Sections';
        var tableID = 'page-sections';
        var presentData = '<table id="' + tableID + '"></table>';
        $.fn.drawContents(currentItem, parentPage, currentPage, presentData, function() {
            $('#data-presentation-span').ready(function() {
                ...
                var excluded = ["_id"];
                var tableInfo = 'd=kayxlavcoms&c=PageSections&t=table&f=';
                $.fn.getTable('PageSections', 'data-presentation-span', 'table table-striped table-bordered bootstrap-datatable datatable responsive dataTable', tableInfo, tableID, '/kayxlav', excluded, defaultSearchMode, opts);
                $('#loading').hide();
        ...
        $('#current-item').val("Pags");
        event.preventDefault();
        var parentPage = 'Configuration';
        var currentPage = 'Pages';
        var currentItem = 'Pages';
        var tableID = 'page';
        var presentData = '<table id="' + tableID + '"></table>';
        $.fn.drawContents(currentItem, parentPage, currentPage, presentData, function() {
            $('#data-presentation-span').ready(function() {
                ...
                var excluded = ["_id"];
                var tableInfo = 'd=kayxlavcoms&c=Pages&t=table&f=';
                $.fn.getTable('Pages', 'data-presentation-span', 'table table-striped table-bordered bootstrap-datatable datatable responsive dataTable', tableInfo, tableID, '/kayxlav', excluded, defaultSearchMode, opts);
                $('#loading').hide();
...
$.fn.redrawClientTable = function(searchStr) {
    ...
    xmlhttp.onreadystatechange = function() {
        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
            ...
            var response = xmlhttp.responseText
            $('#client-table-rows').html(response);;
        }
    };
    xmlhttp.open("GET", "/kayvlav?table_search=clients&search_str=" + searchStr);
    xmlhttp.send();
};

$.fn.getDataFromID = function(tableName, data, id) {
    var idField = "";
    if (tableName === "PageSections") {
        idField = "sectionID";
    }

    var filteredRecord = data.filter(function(itm) {
        return itm[idField] == id;
    });

    console.log('filteredRecord');
    console.dir(filteredRecord);
...
$.fn.confirmFormSubmitDialog = function(header, message, id) {
    ...
    $('#dialog-bttns').html('<button class="btn btn-large btn-danger" onclick="$.fn.loadPreviousPage()" id="dialog-no-bttn">No</button><button class="btn btn-large btn-success" onclick="$.fn.submitForm(\'' + id + '\',\'' + id + '_form_elements\')" id="dialog-yes-bttn">Yes</button>');
    $('#dialog-bttns').show();
...
$(document).ready(function() {
        ...
        if (isFormDataValid === 'YES') {
            ...
            xmlhttp.onreadystatechange = function() {
                if (xmlhttp.readyState === 4 && xmlhttp.status === 200) {
                    ...
                    var respInt = resObj.isSuccessful ? 1 : 0; // parseInt(response);
                    var siteName = $('#site-name').val();
                    var message = replaceAll(resObj.message, '"', '');
                    var mode = resObj.mode;
                    message = replaceAll(message, '\}', '');
                    message = replaceAll(message, '\{', '');
                    message = replaceAll(message, ':', ' ');
                    collectionName = resObj.collection;

                    var headerStr = mode == 1 ? $.fn.capitalizeFirstLetter(collectionName) + " Successfully Updated" : " 1 Item Successfully Added to " + collectionName;

                    if (respInt === 1) {
                        $.session.set(collectionName + "_reset_flag", "1");
                        document.getElementById('dialog-header-span').innerHTML = '<img src="img/kayxlav_medium.jpg" alt="' + siteName + ' logo" /> <span align="center" style="font-weight:bold; font-size:20px;"> ' + headerStr + ' </span>';
                        document.getElementById('dialog-message-div').innerHTML = message;
                        $('#dialog-close-bttn').click(function() {
                            window.location = '#';
                        });
                        $('#dialog-close-bttn').on('click', function() {
                            $.fn.loadPreviousPage();
                        });
                        ...
                        document.getElementById('dialog-header-span').innerHTML = '<img src="img/kayxlav_medium.jpg" alt="' + siteName + ' logo" /> <span align="center" style="font-weight:bold; font-size:20px;">New ' + entity + ' Entry</span>';
    ...
    $.fn.editEntry = function(recordID) {
        //var tableContentsField = document.getElementById('information-table-contents');
        //var tableContents = tableContentsField.value;
        recordInd = parseInt(recordID);

        var tableContents = $('#current-item').val();

        if (tableContents === 'PageSections') {
            ...
            var formDetails = {
                ...
                , name: 'page_section_mode'
                , method: 'post'
                , action: '/kayxlav'
                , type: 'EXISTING'
                , dataType: 'page-section'
                , accessType: 'edit'
                , nextPageUrl: '/kayxlav?d=kayxlavcoms&c=PageSections&f=form'

            };

            var sectionID = new FormElement({
                name: 'sectionID',
                type: 'hidden',
                validation: 'nocheck',
                id: "section-id",
                editable: false,
                value: "",
                class: "input focused", // input-xlarge focused",
                alternativeValues: "",
                chosenValue: "",
                errorMessage: "",
                valueMap: "",
            ...
            var sectionName = new FormElement({
                name: 'sectionName',
                type: 'text',
            ...
            var sectionText = new FormElement({
                name: 'sectionText',
                type: 'text',
            ...
            var creationDate = new FormElement({
                name: 'creationDate',
                type: 'text',
                validation: 'nocheck',
                id: "creation-date",
            ...
            var tableName = 'PageSections';
            var sourceUrl = '/kayxlav';
            var mode = 1;
            var idColumn  = 'sectionID';

            $.fn.getForm(tagID, tableName, sourceUrl, mode, idColumn, recordInd, formDetails, formElements);
  */
            new PageSectionForm('EXISTING', recordID, true, '/kayxlav?d=kayxlavcoms&c=PageSections&f=', 1);
    ...
    $.fn.viewEntry = function(recordInd) {
        ...
        var tableContents = $('#current-item').val();
        //alert('Viewing  record ' + recordInd + ' of ' + tableContents);

        $.fn.setLoadingDialog();
        $.fn.showDialog("Loading  details for  selected  item in " + tableContents);
        ...
        if (tableContents === 'PageSections') {
            var formDetails = {
                ...
                dataType: 'page-section',
                accessType: 'view',
                nextPageUrl: '/kayxlav?d=kayxlavcoms&c=PageSections&f='
            ...
            var sectionID = new FormElement({
                ...
                editable: false,
                value: "",
                class: "input disabled", // input-xlarge focused",
                alternativeValues: "",
                chosenValue: "",
                errorMessage: "",
                valueMap: "",
            ...
            var mode = 0;
            var resetFlag = 1;
            var sectionIDCol = 'sectionID';


            $.fn.getForm(tagID, tableName, sourceUrl, mode, sectionIDCol, recordInd, formDetails, formElements, resetFlag);
        ...
        xmlhttp.onreadystatechange = function() {
            ...
            if (xmlhttp.readyState === 4 && xmlhttp.status === 200) {
                ...
                if (response.toLowerCase().indexOf('success') >= 0) {
                    ...
                    document.getElementById('dialog-header-span').innerHTML = '<img src="img/kayxlav_medium.jpg" alt="Unna logo" /> <span align="center" style="font-weight:bold; font-size:20px;float: center;">Successfully removed client: <br /><span align="center" >' + clientName + '</span></span>';
                    ...
                    //  $('#myModal').modal('hide');                             
                    document.getElementById('dialog-header-span').innerHTML = '<img src="img/kayxlav_medium.jpg" alt="Unna logo" /> <span align="center" style="font-weight:bold; font-size:20px;float: center;">Unable to delete client' + clientName + '</span>';
                    document.getElementById('dialog-message-div').innerHTML = response;
                    $('#dialog-close-bttn').show();
                    $('#myModal').modal('show');
                }
        ...
        xmlhttp.onreadystatechange = function() {
            ...
            if (xmlhttp.readyState === 4 && xmlhttp.status === 200) {
                ...
                if (response.toLowerCase().indexOf('success') >= 0) {
                    ...
                    document.getElementById('dialog-header-span').innerHTML = '<img src="img/kayxlav_medium.jpg" alt="Unna logo" /> <span align="center" style="font-weight:bold; font-size:20px;float: center;">Successfully removed Setting: <br /><span align="center">' + settingName + '</span></span>';
                    document.getElementById('dialog-message-div').innerHTML = '<div align="center">' + response + '</div>';
                    $('#dialog-close-bttn').show();
                    $('#myModal').modal('show');
                } else {
                    ...
                    document.getElementById('dialog-header-span').innerHTML = '<img src="img/kayxlav_medium.jpg" alt="Unna logo" /> <span align="center" style="font-weight:bold; font-size:20px;float: center;">Unable to delete Setting: ' + settingName + '</span>';
                    document.getElementById('dialog-message-div').innerHTML = response;
                    $('#dialog-close-bttn').show();
                    $('#myModal').modal('show');
                }
        ...
        xmlhttp.onreadystatechange = function() {
            ...
            if (xmlhttp.readyState === 4 && xmlhttp.status === 200) {
                ...
                if (response.toLowerCase().indexOf('success') >= 0) {
                    ...
                    $.fn.redrawSettingTable('%');
                    document.getElementById('dialog-header-span').innerHTML = '<img src="img/kayxlav_medium.jpg" alt="Unna logo" /> <span align="center" style="font-weight:bold; font-size:20px;float: center;">Successfully removed Email Profile: <br /><span align="center">' + profileName + '</span></span>';
                    document.getElementById('dialog-message-div').innerHTML = '<div align="center">' + response + '</div>';
                    $('#dialog-close-bttn').show();
                    $('#myModal').modal('show');
                    ...
                    document.getElementById('dialog-header-span').innerHTML = '<img src="img/kayxlav_medium.jpg" alt="Unna logo" /> <span align="center" style="font-weight:bold; font-size:20px;float: center;">Unable to delete Profile: ' + profileName + '</span>';
                    document.getElementById('dialog-message-div').innerHTML = response;
                    $('#dialog-close-bttn').show();
                    $('#myModal').modal('show');
                }
    ...
    $.fn.deleteUser = function(userName, userID, userIndex) {
        ...
        xmlhttp.onreadystatechange = function() {
            ...
            if (xmlhttp.readyState === 4 && xmlhttp.status === 200) {
                ...
                if (response.toLowerCase().indexOf('success') >= 0) {
                    $.fn.redrawUserTable('%');
                    document.getElementById('dialog-header-span').innerHTML = '<img src="img/kayxlav_medium.jpg" alt="Unna logo" /> <span align="center" style="font-weight:bold; font-size:20px;float: center;">Successfully removed User: <br /><span align="center">' + userName + '</span></span>';
                    document.getElementById('dialog-message-div').innerHTML = '<div align="center">' + response + '</div>';
                    $('#dialog-close-bttn').show();
                    $('#myModal').modal('show');
                } else {
                    document.getElementById('dialog-header-span').innerHTML = '<img src="img/kayxlav_medium.jpg" alt="Unna logo" /> <span align="center" style="font-weight:bold; font-size:20px;float: center;">Unable to delete User: ' + userName + '</span>';
                    document.getElementById('dialog-message-div').innerHTML = response;
                    $('#dialog-close-bttn').show();
                    $('#myModal').modal('show');
                }
    ...
    $('#dialog-username-id').on('change', function() {
        ...
        if (username !== loginUser) {
            ...
            $('#dialog-password-id').prop('class', 'form-control');
    ...
    $('#dialog-login-form').on('submit', function(e) {
        ...
        if ((username === null || username === '') || (password === null || password === '')) {
            ...
            if (username === null || username === '') {
                ...
                usernameDiv.className = "form-group error";
            ...
            if (password === null || password === '') {
                ...
                passwordDiv.className = "form-group error";
    ...
    $.fn.formValidator = function(formId, elementsStr) {
        ...
        for (var field in formFields) {
            if (field) {
                ...
                field.value = value;
                messageID = fieldName.toLowerCase() + '_message';
                nameOnly = fieldName.replace('_element', '').replace('_', ' ');
                nameOnly = nameOnly.replace('_', ' ');
                fieldDivStr = fieldName.toLowerCase() + '_div';
                ...
                switch (fieldValidation) {
                    ...
                    case 'alphanumeric':
                        if (value.length === 0) {
                            ...
                            $("#" + messageID).show();
                            formInputfieldDiv.className = "form-group error";
                            ...
                            if (isAlphanumeric(field)) {
                                ...
                                $("#" + messageID).val('');
                                $("#" + messageID).hide();
                                formInputfieldDiv.className = "form-group";
                            } else {
                    ...
                    case 'password':
                        ...
                        if (confirmValue && confirmValue.length === 0) {
                            ...
                            confirmFormInputfieldDiv.className = "form-group error";
                            confirmFieldValidationMessage = '<li>Password confirmation is required</li>';
                        } else {
                            $("#" + confirmMessageID).hide();
                            confirmFormInputfieldDiv.className = "form-group";
                        }
                        if (value.length > 0 && confirmValue.length > 0) {
                            if ((confirmValue === value) && (value.length >= minPasswordLen)) {
                                if (isValidPass(field)) {
                                    isValid = true;
                                    $("#" + messageID).hide();
                                    formInputfieldDiv.className = "form-group success";
                                    $("#" + confirmMessageID).hide();
                                    confirmFormInputfieldDiv.className = "form-group success";
                                } else {
                                    $("#" + messageID).show();
    ...
    $.fn.showValidationReport = function(formId, validationResponses, formTarget, nextPageBttn) {
        ...
        if (!shouldProceed) {
            ...
            var logo = $('#site-logo').val();
            var siteName = $('#site-name').val();
            document.getElementById('dialog-header-span').innerHTML = '<img src="' + logo + '" alt="' + siteName + '" /> <span align="center" style="font-weight:bold; font-size:20px;">New ' + $.fn.splitnCapitalizeFirstLetter(formId, '-') + ' Entry</span>';
    ...
    function body(columns, tableData) {
        ...
        for (var i = 0; i < tData.length; i++) {
            ...
            bodyStr += ' <td class="center" > <div align="center"><a class="btn btn-success" href="#" onclick="$.fn.viewEntry(\'' + tData.length + '\')">' +
                '<i class="glyphicon glyphicon-zoom-in icon-white"></i> View</a></div></td><a class="btn btn-danger" href="#" ' +
                ' onclick="$.fn.deleteEntry(\'' + tData[i][1] + '\',\'' + tData[i][1] + '\',' + i + ')"><i class="glyphicon glyphglyphicon glyphicon-edit icon-white"></i>Delete</a></tr>';
        }
        bodyStr += '</tbody></table>';
        return bodyStr;
    };
    ...
    var TablePrototype = {
        ...
        buildHeader: function(className, id, columns, tDataCount, excludedCols) {
            ...
            for (var j = 0; j < columns.length; j++) {
                ...
                if (excludedCols.indexOf(columns[j]) === -1) {
                    column = columns[j].toUpperCase();
                    headerString += '<th>' + replaceAll(column, '_', ' ') + '</th>';
                }
            }
        ...
        buildBody: function body(columns, tableData, excludedCols, opts) {
            //   var  tColumns =  columns;
            //  var  tData= tableData;
            var bodyStr = '<tbody role="alert">';
            var searchStr = '';
            var size = (tableData) ? tableData.length : 0;
            for (var i = 0; i < size; i++) {
                rowShading = i % 2 == 0 ? "even" : "odd";
                for (var j = 0; j < columns.length; j++) {
                    searchStr += tableData[i][columns[j]] + ' ';

                }
                bodyStr += '<tr class="' + rowShading + '">';
                ...
                if (opts.isViewable) {
                    bodyStr += '<a class="btn btn-success" href="#" onclick="$.fn.viewEntry(\'' + tableData[i][columns[1]].toString() + '\')">' +
                        '<i class="glyphicon glyphicon-zoom-in icon-white"></i>View</a>';
                }
                if (opts.isEditable) {
                    bodyStr += '<a class="btn btn-info" href="#" onclick="$.fn.editEntry(\'' + tableData[i][columns[1]].toString() + '\')">' +
                ...
                if (!opts.isIndelible) {
                    ...
                    ' onclick="$.fn.deleteEntry(\'' + tableData[i][columns[1]] + '\',\'' + tableData[i][columns[2]] + '\',' + i + ')">' +
                        '<i class="glyphicon glyphglyphicon glyphicon-edit icon-white"></i>Delete</a>';
                }
                bodyStr += '</td></tr>';
            }
            bodyStr += '</tbody></table>';
...
'</div>  '; */
    ...
    $.fn.getTable = function(tableName, tagID, className, tableParameters, tableID, tableUrl, excludedCols, mode, opts) {

        var responseData = '';
        var xmlhttp = null;

        var tableSessionData = $.session.get(tableName) ? $.session.get(tableName) : "";
        ...
        var tablResetFlag = $.session.get(tableName + "_reset_flag") == 1 ? true : false;
        if (tableSessionData.length > 1 && mode === 0 && !tablResetFlag) {

            responseData = JSON.parse(tableSessionData);
            tab.columns = responseData.columns;
            tab.tData = responseData.tabData.length > 0 ? responseData.tabData : [];
            tab.dataCount = responseData.dataCount;
            var header = tab.buildHeader(className, tableID, tab.columns, tab.dataCount, excludedCols);
            var body = tab.buildBody(tab.columns, tab.tData, excludedCols, opts);
                ...
                if (xmlhttp.readyState === 4 && xmlhttp.status === 200) {
                    ...
                    try {
                        ...
                        if (responseData.tabData.length > 0) {
                            $.session.set(tableName + "_reset_flag", "0");
                            var tab = new Table(tableName, tableID);
                            tab.columns = responseData.columns;
                            tab.tData = responseData.tabData.length > 0 ? responseData.tabData : [];
                            tab.dataCount = responseData.dataCount;
                        ...
                        $.fn.showMessageDialog('<div align="center" style="font-weight:bold; font-decoration:underline"  >Data Fetch Error</div>', '<div align = "left" style="foreground:red; ">Error Comminucating with Server' + e.stack + '</div><br /><br /><span align="left" style="foreground:blue"> ' + xmlhttp.responseText + '</span>');
                    }
                }
            };
            xmlhttp.open("POST", tableUrl, true);
            xmlhttp.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
            xmlhttp.send(tableParameters);
        }
    };
    ...
    var formElementPrototype = {
        ...
        render: function(valueData) {
            ...
            if (this.type === 'text' && this.editable) {
                fieldStr = '<div class="form-group"  id="' + norm_div_id + '"><label class="control-label" for="' + id + '">' + field + '</label>' +
                    '<input class="form-control" id="' + id + '" type="text" name="' + input_name + '" value="' + valueData + '"/>' +
                    '<span class="help-inline" style="display:none;" id="' + message_id + '">This value is not allowed<br /></span></div>';
            } else if (this.type === 'text' && !this.editable) {
                fieldStr = '<div class="form-group"  id="' + norm_div_id + '"><label class="control-label" for="' + id + '">' + field + '</label>' +
                    '<input class="form-control disabled" type="text" disabled="" placeholder="' + valueData + '" id="' + id + '" name="' + input_name + '" value="' + valueData + '">' +
                    '<span class="help-inline" style="display:none;" id="' + message_id + '">This value is not allowed</span></div> ';
                ...
                fieldStr = '<div class="form-group"  id="' + norm_div_id + '"><label class="control-label" for="' + id + '">' + field + '</label>' +
                    '<div class="controls"><input  class="form-control" id="' + id + '" type="password" name="' + input_name + '" value="' + valueData + '"/>' +
                ...
                fieldStr = '<div class="form-group"  id="' + norm_div_id + '"><label class="control-label" for="' + id + '">' + field + '</label><div class="controls"> <textarea  class="span10" name="' + input_name + '" class="cleditor" id="' + id + '" rows="3">' + valueData + '</textarea>' +
                    '<span class="help-inline" style="display:none;" id="' + message_id + '">This value is not allowed<br /></span></div></div>';
                ...
                fieldStr = '<div class="form-group"  id="' + norm_div_id + '"><label class="control-label" for="' + id + '">' + field + '</label>' +
                    '<div class="controls"><input class="form-control typeahead"   id="' + id + '" type="text" data-provide="' + id + '" data-items="4"  name="' + input_name + '" value="' + valueData + '" data-source="' + datasource + '">' +
                    '<span class="help-inline" style="display:none;" id="' + message_id + '">This value is not allowed<br /></span> </div></div>';

            } else if (this.type === 'datepicker') {
                fieldStr = '<div class="form-group"  id="' + norm_div_id + '"><label class="control-label" for="' + id + '">' + field + '</label>' +
                    '<div class="controls"><input class="form-control datepicker" id="' + id + '" type="text" name="' + input_name + '" value="' + valueData + '"/>' +
                    '<span class="help-inline" style="display:none;" id="' + message_id + '">This value is not allowed<br /></span> </div></div>';

            } else if (this.type === 'file') {

                fieldStr = '<div class="form-group" id="' + norm_div_id + '"> <label class="control-label" for="fileInput">' + field + '</label><div class="controls">' +
                ...
                fieldStr = '<div class="form-group" id="' + norm_div_id + '"><label class="control-label" for="' + id + '">' + field + '</label><div class="controls">' +
                ...
                if (this.editable) {
                    fieldStr = '<div class="form-group" id="' + norm_div_id + '" valign="center"><label class="control-label" for="' + id + '" >' + field + '</label>' +
                        '<div class="controls">' +
    ...
    var formPrototype = {
        ...
        foot: "",
        headerStr: function() {
            var headerStr = '<form role="form" class="' + this.class + '" enctype="' + this.encType + '"  id ="' + this.id + '" name="' + this.name + '" method ="' + this.method + '" action="' + this.action + '" >' +
        ...
        footerStr: function() {
            ...
            if (this.accessType === 'edit' || this.type === 'NEW') {
                footerStr += '<div align="center" id="' + this.id + '-footer-section"><button type="submit" class="btn btn-primary" id="submit-bttn" onclick = "$.fn.confirmFormSubmitDialog(\'Saving Changes\',\'Are you sure you want to save?\',\'' + this.id + '\')" >Save</button>' +
                    ...
                    '<button class="btn" id="cancel-bttn" type="reset" onclick="$.fn.loadPreviousPage();" >Cancel</button> </div>';
            } else if (this.accessType === 'view') {
                footerStr += '<div align="center" id="' + this.id + '-footer-section"><a align="center" href = "#" onclick="$.fn.loadPreviousPage();" class="btn btn-primary" id="' + this.id + '-footer-section-link"> Back</a></div>';
            }
    ...
    $.fn.getForm = function(tagID, tableName, sourceUrl, mode, idColumn, recordID, formDetails, elements, resetFlag = 0) {

        var responseData = '';
        var paramString = 'c=' + tableName + '&t=form&f=' + idColumn + '&id_value=' + recordID;
        var formTarget = sourceUrl;
        var xmlhttp = null;
        var formSessionData = $.session.get(tableName);
        var formData = "";

        //   alert(getDate() + "> resetFlag: " + resetFlag);
        //var resetFlag = 0;

        // alert('formDetails.type: ' + formDetails.type + '<br />mode: ' + mode + '<br />formSessionData:' + formSessionData + '<br />resetFlag:' + resetFlag);
        ...
        if (formDetails.type === 'NEW' && mode === 1) {
            ...
            docReady();
            //   }
        } else if (resetFlag == 0 && formDetails.type === 'EXISTING' && mode === 0) { //((formSessionData) !== null && ) {
            try {
                responseData = JSON.parse(formSessionData);
                if (responseData.tabData.length > 0) {
                    var currentForm = new Form();
                    currentForm.class = formDetails.class;
                    currentForm.id = formDetails.id;
                    ...
                    var p = 0;
                    currentRecord = $.fn.getDataFromID(tableName, currentForm.data, recordID);
                    // console.log('currentRecord');
                    //console.dir(currentRecord);
                    for (var index in elements) {

                        curElement = elements[index].name;
                        // alert('current element: ' + curElement + ' value = ' + currentRecord[curElement]);
                        if (currentRecord[curElement]) {
                            elements[index].value = currentRecord[curElement].toString().replace('null', '');
                            elements[index].render(currentRecord[curElement].toString().replace('null', ''));
                        } else {
                            elements[index].value = '';
                            elements[index].render('');
                    ...
                    $.fn.showMessageDialog('Memory Cache for Form is empty', 'There  is no data stored for this form');
                }


            } catch (e) {
                $.fn.showMessageDialog('Error loading data from memory', e.stack);
            }



        } else if (resetFlag == 0 && formDetails.type === 'EXISTING' && mode === 1) { //((formSessionData) !== null && resetFlag !== 1) {
            try {
                responseData = JSON.parse(formSessionData);
                if (responseData.tabData.length > 0) {
                    var currentForm = new Form();
                    ...
                    for (var index in elements) {
                        ...
                        if (currentRecord[curElement]) {
                            ...
                            if (!(elements[index].name.toString().toLowerCase().endsWith("id") || elements[index].name.toString().toLowerCase().endsWith("date") || elements[index].name.toString().toLowerCase().endsWith("time"))) elements[index].editable = true;
                            elements[index].value = currentRecord[curElement].toString().replace('null', '');
                            elements[index].render(currentRecord[curElement].toString().replace('null', ''));
                        } else {
                            elements[index].value = '';
                ...
                if (xmlhttp.readyState === 4 && xmlhttp.status === 200) {
                    try {
                        ...
                        if (responseData.tabData.length > 0) {
                            ...
                            $.fn.showMessageDialog('Unable to fetch data for  form', 'Server response: ' + xmlhttp.responseText);
                        }
    ...
    if (tableContents === 'setting') $.fn.getTable('data-presentation-span', 'table table-striped table-bordered bootstrap-datatable datatable dataTable no-footer', 'nst_settings', 'settings_table', '/kayxlav', excluded, defaultSearchMode);
    ...
    $.fn.drawContents = function(currentItem, parentPage, currentPage, presentationData, callback) {
        ...
        var contentStr = '<div id="content" >' +
            ...
            '' +
            '<a id = "new-' + currentItem.toLowerCase() + '" data-original-title="New ' + currentItem + '" href="#" class="btn btn-large btn-success" data-rel="popover" data-content="Click here to add a new ' + currentItem + '" onClick="$.fn.addNewItem(this)"><i class="glyphicon glyphicon-plus icon-white" id="new-item" ></i> New ' + currentItem.substring(0, currentItem.length - 1) + '</a>' +
            '</div>' +
            ...
            '<h2> <i class="glyphicon glyphicon-edit"></i> ' + currentItem + '</h2>' +
            '<div class="box-icon">' +
            ...
            '<div  id="data-presentation-span" class="row-fluid sortable"><div>' + presentationData +
            '</div>' +
            '</div>' +
            '</div>' +
            '</div>' +
            ...
            '<!-- content ends -->' +
            ' </div>';
        $('#content').fadeIn("fast").html(contentStr);
        callback();
    ...
    $.fn.drawPurchaseTable = function(tableInfo, excluded, defaultSearchMode, opts) {
        ...
        $.fn.getTable('data-presentation-span', 'table table-striped table-bordered bootstrap-datatable datatable responsive datatable', tableInfo, 'purchase_table', '/kayxlav', excluded, defaultSearchMode, opts);


    };
    $.fn.drawRoleTable = function(tableInfo, excluded, defaultSearchMode, opts) {

        $.fn.getTable('data-presentation-span', 'table table-striped table-bordered bootstrap-datatable datatable responsive datatable', tableInfo, 'role_table', '/kayxlav', excluded, defaultSearchMode, opts);
    };
    $.fn.drawSupplyTable = function(tableInfo, excluded, defaultSearchMode, opts) {

        $.fn.getTable('data-presentation-span', 'table table-striped table-bordered bootstrap-datatable datatable responsive datatable', tableInfo, 'supply_table', '/kayxlav', excluded, defaultSearchMode, opts);


    };
    $.fn.drawProductTable = function(tableInfo, excluded, defaultSearchMode, opts) {

        $.fn.getTable('data-presentation-span', 'table table-striped table-bordered bootstrap-datatable datatable responsive datatable', tableInfo, 'product_table', '/kayxlav', excluded, defaultSearchMode, opts);

    };

    $.fn.drawUserTable = function(tableInfo, excluded, defaultSearchMode, opts) {

        $.fn.getTable('data-presentation-span', 'table table-striped table-bordered bootstrap-datatable datatable responsive datatable', tableInfo, 'user_table', '/kayxlav', excluded, defaultSearchMode, opts);
    ...
    $.fn.getPurchaseStats = function(dateField, startDate, endDate) {
        ...
        $.post("/kayxlav", { "reqType": 'misc', "dataType": 'purchase', "f": dateField, "s": startDate, "e": endDate })
            .done(function(data) {
                try {
    ...
    $.fn.getSupplyStats = function(dateField, startDate, endDate) {
        ...
        $.post("/kayxlav", { "reqType": 'misc', "dataType": 'supply', "f": dateField, "s": startDate, "e": endDate })
            .done(function(data) {
                try {
                    data = JSON.parse(data);
                    var unpaidCount1 = 0;
    ...
    $.fn.getAllProducts = function() {
        ...
        $.post("/kayxlav", { "reqType": 'misc', "dataType": 'products' })
            .done(function(data) {
                var products = new Array();
                try {
    ...
    $.fn.getAllRoles = function() {
        ...
        var roles = new Array();
        $.post("/kayxlav", { "reqType": 'misc', "dataType": 'roles' })
            .done(function(data) {
                var roles = new Array();
                try {
                    if (data.length && data !== null && data !== "") {
    ...
    class PageSectionForm {
        ...
        constructor(type, recordID, isEditable, nextPage, resetFlag = 0) {

            this.access = !isEditable ? 'view' : 'edit';
            this.formDetails = {
                class: '',
                encType: 'multipart/form-data',
                id: 'page-section-form',
                ...
                nextPageUrl: nextPage //'/kayxlav?d=kayxlavcoms&c=PageSections&f='

            };
            this.sectionID = new FormElement({
                name: 'sectionID',
                type: 'text',
                validation: 'numeric',
                id: "page-section-form",
            ...
            this.sectionName = new FormElement({
                name: 'sectionName',
                type: 'text',
                validation: 'alphanumeric',
                size: 0,
            ...
            this.sectionText = new FormElement({
                name: 'sectionText',
                type: 'text',
                validation: 'alphanumspecial',
                id: "",
            ...
            this.creationDate = new FormElement({
                name: 'creationDate',
                type: 'text',
                validation: 'email',
                id: "",
            ...
            this.tagID = 'data-presentation-span';
            this.tableName = 'PageSections';
            this.sourceUrl = '/kayxlav';
            this.sectionIDCol = 'sectionID';
            this.mode = type === 'NEW' ? 1 : 0;
            $.session.set('previous_page', $('#content').html());
            this.recordInd = recordID;
            $.fn.getForm(this.tagID, this.tableName, this.sourceUrl, this.mode, this.sectionIDCol, this.recordInd, this.formDetails, this.formElements, this.resetFlag);
